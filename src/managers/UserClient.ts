...
export class UserClient {
	...
	public async getPlaylists(
		...
		const playlists: Array<Playlist> = fetchedData.items,
			length = playlists.length;

		if (fetchAll && fetchedData.total > length) {
			for (let offset = length; offset < fetchedData.total; offset += length) {
				const playlistBatch: Paging<Playlist> = await this.client.fetch("/me/playlists", {
					params: { limit: length, offset },
				});
	 ...
	 */
	public async getSavedAlbums(options: {
		limit?: number;
		offset?: number;
		market?: string;
	}): Promise<Saved<Album>[]> {
	...
	public hasAlbums(...ids: string[]): Promise<boolean[]> {
		return this.client
			.fetch("/me/albums/contains", { params: { ids: ids.join(",") } })
			.then(x => x || ids.map(() => false));
	}
	 ...
	 */
	public async getSavedTracks(options: {
		limit?: number;
		offset?: number;
		market?: string;
	}): Promise<Saved<Track>[]> {
	...
	public hasTracks(...ids: string[]): Promise<boolean[]> {
		return this.client
			.fetch("/me/tracks/contains", { params: { ids: ids.join(",") } })
			.then(x => x || ids.map(() => false));
	}
	 ...
	 */
	public async getSavedEpisodes(options: {
		limit?: number;
		offset?: number;
		market?: string;
	...
	public hasEpisodes(...ids: string[]): Promise<boolean[]> {
		return this.client
			.fetch("/me/episodes/contains", { params: { ids: ids.join(",") } })
			.then(x => x || ids.map(() => false));
	}
	 ...
	 */
	public async getSavedShows(options: { limit?: number; offset?: number; market?: string }): Promise<Saved<Show>[]> {
	...
	public hasShows(...ids: string[]): Promise<boolean[]> {
		return this.client
			.fetch("/me/shows/contains", { params: { ids: ids.join(",") } })
			.then(x => x || ids.map(() => false));
	}
}
