 ...
 */
const encodeBase64 = typeof Buffer === "undefined" ? btoa : buffer => Buffer.from(buffer).toString("base64");
...
export class AuthManager {
	...
	public async getUserToken(options: GetUserTokenOptions): Promise<UserTokenContext> {
		...
		const grant_type = options.refreshToken && !options.code ? "refresh_token" : "authorization_code";
